...
preProcessFCS_center <- function(inputPath, center){
  ...
  store.allFCS <- NULL
    ...
    if(center == "gothenburg"|center =="helsinki"|center=="umea"|center=="Israel"
       |center=="karolinska"){
    ...
    if(center == "copenhagen"){
      ...
      store.allFCS.temp <- cbind(store.allFCS.temp,gsub(".fcs","",sapply(1:length(store.allFCS.temp[,4]),function(x){
        substr(unlist(strsplit(store.allFCS.temp[,4][x],split = store.allFCS.temp[,5][x]))[2], 2,1000000000)})))
